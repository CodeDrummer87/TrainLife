...
const remove_image_button = document.getElementById('remove_image_button');
const loco_photo = document.getElementById('loco_photo');
let file;

loco_photo.onmouseover = function () {
    if (!isUserPhotoLoaded)
        loco_photo.src = '../images/placeholder_hover.png';
}
loco_photo.onmouseout = function () {
    if (!isUserPhotoLoaded)
        loco_photo.src = '../images/placeholder.png';
}

loco_photo.onclick = async function() {
    ...
    fileInput.onchange = async function(e) {
        ...
        if (!file.type.match('image.*')) {
            displayMessage('Файл должен быть изображением', false);
            return;
        }
        if (file.size > 10 * 1024 * 1024) {
            displayMessage('Допустимый размер файла: 10MB', false);
            return;
        }

        const imgUrl = URL.createObjectURL(file);
        isUserPhotoLoaded = true;
        loco_photo.src = imgUrl;
        remove_image_button.innerText = 'убрать фото';
        sessionStorage.setItem('loco-photo', imgUrl);
        //await sendToServer(file);
    }
}

remove_image_button.onclick = function() {
    file = undefined;
    isUserPhotoLoaded = false;
    URL.revokeObjectURL(loco_photo.src);
    loco_photo.src = '../images/placeholder.png';
    sessionStorage.removeItem('loco-photo');
    remove_image_button.innerText = '\u00A0';
}

async function sendToServer(file) {
    if (!file) {
        displayMessage('Файл не выбран', false);
        return;
    }

    const formData = new FormData();
    formData.append('file', file);
    formData.append('description', 'тут будет описание'); //.:: TODO::::::::::::::::::::::::::::::::
    try {
        const response = await fetch(url + '/photo/upload', {
            method: 'POST',
            body: formData
        });
        ...
        displayMessage('Ошибка загрузки фотографии', false);
    }
}
//endregion
...
s_attendance.onclick = function () {
    ...
    input_time.value = sessionStorage.getItem('attendance') === null ?
        getCurrentDateAndTime() : sessionStorage.getItem('attendance');
